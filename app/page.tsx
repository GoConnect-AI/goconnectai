...
import { Check, ArrowRight, Sparkles, Phone, Mail, MapPin, Shield, Users, Layers, LineChart, Clock, Rocket, Workflow, Building2 } from 'lucide-react'

const CONFIG = {
  brand: {
    name: 'GoConnectAI',
    tagline: 'Grow revenue on autopilot — AI receptionists plus full-funnel automation for lead gen, reactivation, and retention.',
  ...
  services: [
    ...
    { icon: <Rocket className="h-6 w-6"/>, title: 'Growth Systems', desc: 'Automation for reactivation, retention, and lead generation that fuels predictable revenue.', points: ['Marketing reactivation', 'Customer retention', 'Lead generation automations'] }
  ],
  process: [
    ...
    <div className="min-h-screen text-foreground">
      <header className="sticky top-0 z-40 w-full backdrop-blur bg-white/70 border-b">
        <Section className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            <a href="#services" className="text-sm text-gray-500 hover:text-black transition-colors">Solutions</a>
            <a href="#process" className="text-sm text-gray-500 hover:text-black transition-colors">Process</a>
          ...
          <Button asChild size="sm"><a href={CONFIG.brand.ctaPrimary.href}>{CONFIG.brand.ctaPrimary.label}</a></Button>
        </Section>
      </header>
      ...
      <Section className="pt-20 pb-16">
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          <FadeIn>
            <h1 className="text-4xl md:text-5xl font-bold leading-tight">{CONFIG.brand.tagline}</h1>
            <p className="mt-4 text-lg text-gray-600">We combine AI, automation, and data to help you grow revenue and streamline operations — without extra staff.</p>
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <Button asChild size="lg" className="rounded-2xl"><a href={CONFIG.brand.ctaPrimary.href}>{CONFIG.brand.ctaPrimary.label}<ArrowRight className="ml-2 h-4 w-4" /></a></Button>
              <Button asChild variant="outline" size="lg" className="rounded-2xl"><a href={CONFIG.brand.ctaSecondary.href}>{CONFIG.brand.ctaSecondary.label}</a></Button>
            </div>
          </FadeIn>
          <FadeIn delay={0.1}>
            <Card className="rounded-2xl shadow-lg">
              <CardHeader><CardTitle className="flex items-center gap-2"><Shield className="h-5 w-5"/>Quality by design</CardTitle></CardHeader>
      ...
      <Section id="services" className="py-16">
        <FadeIn><h2 className="text-3xl font-bold">Solutions</h2><p className="mt-2 text-gray-600">From AI receptionists to marketing automation — everything connects.</p></FadeIn>
        <div className="mt-8 grid md:grid-cols-3 gap-6">
          {CONFIG.services.map((s) => (
            <FadeIn key={s.title}>
              <Card className="rounded-2xl h-full">
                <CardHeader><CardTitle className="flex items-center gap-2">{s.icon}{s.title}</CardTitle></CardHeader>
                <CardContent><p className="text-sm text-gray-600">{s.desc}</p><ul className="mt-4 space-y-2 text-sm">{s.points.map((pt) => (<li key={pt} className="flex items-center gap-2"><Check className="h-4 w-4"/>{pt}</li>))}</ul></CardContent>
              </Card>
            </FadeIn>
          ))}
        </div>
      ...
      <Section id="process" className="py-16">
        <FadeIn><h2 className="text-3xl font-bold">Process</h2><p className="mt-2 text-gray-600">Simple steps, fast results.</p></FadeIn>
        <div className="mt-8 grid md:grid-cols-4 gap-6">
          {CONFIG.process.map((step) => (
            <FadeIn key={step.title}><Card className="rounded-2xl h-full"><CardHeader><CardTitle className="flex items-center gap-2">{step.icon}{step.title}</CardTitle></CardHeader><CardContent><p className="text-sm text-gray-600">{step.desc}</p></CardContent></Card></FadeIn>
          ))}
        </div>
      </Section>

      <Section id="pricing" className="py-16">
        <FadeIn><h2 className="text-3xl font-bold">Pricing</h2><p className="mt-2 text-gray-600">Transparent packages for any scale.</p></FadeIn>
        <div className="mt-8 grid md:grid-cols-2 gap-6">
          {CONFIG.pricing.map((tier) => (
            <FadeIn key={tier.name}><Card className={`rounded-2xl h-full ${tier.highlighted ? 'border border-black shadow-lg' : ''}`}><CardHeader><CardTitle className="flex items-center justify-between">{tier.name}{tier.highlighted && <Badge>Popular</Badge>}</CardTitle></CardHeader><CardContent className="space-y-4"><div className="text-3xl font-bold">{tier.price}</div><p className="text-sm text-gray-600">{tier.blurb}</p><ul className="space-y-2 text-sm">{tier.features.map((f) => (<li key={f} className="flex items-center gap-2"><Check className="h-4 w-4"/>{f}</li>))}</ul><Button className="w-full rounded-2xl">{tier.cta}</Button></CardContent></Card></FadeIn>
          ))}
        </div>
      </Section>

      <Section id="faq" className="py-16">
        <FadeIn><h2 className="text-3xl font-bold">FAQ</h2><p className="mt-2 text-gray-600">If it’s not here, ask below.</p></FadeIn>
        <div className="mt-8 grid md:grid-cols-3 gap-6">
          {CONFIG.faq.map((f, i) => (
            <FadeIn key={i}><Card className="rounded-2xl h-full"><CardHeader><CardTitle>{f.q}</CardTitle></CardHeader><CardContent><p className="text-sm text-gray-600">{f.a}</p></CardContent></Card></FadeIn>
          ))}
        </div>
      </Section>
      ...
      <footer className="border-t bg-white/50">
        <Section className="py-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-sm text-gray-600"><Building2 className="h-4 w-4"/><span>© {new Date().getFullYear()} {CONFIG.brand.name}. All rights reserved.</span></div>
          <div className="flex items-center gap-4 text-sm"><a href="#" className="hover:underline">Privacy</a><a href="#" className="hover:underline">Terms</a></div>
        </Section>
      </footer>
    </div>
  )
...
function FeatureLine({ text }: { text: string }) {
  return (<div className="flex items-center gap-2 text-sm"><Check className="h-4 w-4" /><span>{text}</span></div>)
}
